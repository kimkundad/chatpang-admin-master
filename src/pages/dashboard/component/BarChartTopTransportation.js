...
const BarChartTopTransportation = (props) => {
    ...
    return (
        <ResponsiveBar
            colors={["#4360A8", "#7197DF"]}
            // colorBy="index"
            data={data?.length > 0 ? data : sampleData}
            layout="horizontal"
            keys={[
                'confirm', 'total_sub_confirm'
            ]}
            indexBy={indexBy}
            ...
            tooltip={(props) => {
                ...
                let showVal
                // if (props.value == props.data.amount_sub_total)
                //     showVal = Math.round(props.data.amount)
                // else
                //     showVal = Math.round(props.value)

                showVal = `${props.indexValue} : เรียกรถ : ${props.data.total} | ยืนยันแล้ว : ${props.data?.confirm ? props.data?.confirm : 0}`
            ...
            label={(d) => {
                // console.log("TopWalletLabel", d)
                // return <tspan x={20}>{d.value}</tspan>
                let showVal
                if (d.id == "total_sub_confirm")
                    showVal = Math.round(d.data.total)
                else
                    showVal = Math.round(d.value)
                return <tspan fill="white">{fcomma(showVal)}</tspan>
...
export default BarChartTopTransportation;