...
// dashboardApi.util.invalidateTags(['OrderItems','Recipient','Orders'])
...
const FilterData = ({ onSearch, pageCode, exportFunction }) => {
  ...
  // const { createdChartTime } = useSelector((state) => state.dashboardReducer);
  ...
  useEffect(() => {
    console.log("createdAt", startDate, endDate, allAgency)
    // dispatch(allAction.storeListSellAction.getMasterLevel())
    //   .then()
    //   .catch((e) => message.error(e.message));
    // const std = moment(new Date(), 'YYYY-MM-DD').subtract(1, 'days');
    ...
    <>
      ...
      >
        <Row gutter={[8, 4]}>
          {FilterUserLevel(['SAD']) && (
            <Col flex="1 1 100px">
              <Form.Item label={t('company-select')} name="companyId" initialValue={companyId}>
                <Select
                  // allowClear
                  showSearch
                  ...
                  onChange={(e) => {
                    ...
                    dispatch(allAction.dashboardAction.setCreatedChartTime(
                      form.getFieldsValue()
                    ))
                  }}
          ...
          {FilterUserLevel(['SAD', 'COM']) && (
            <Col flex="1 1 10px">
              <Form.Item label={"Type"} name="allAgency" initialValue={allAgency}>
                <Select
                  // allowClear
                  // showSearch
                  // defaultValue={allAgency}
                  ...
                  filterOption={(input, option) => option.children
                    ...
                    .indexOf(input.toLowerCase()) >= 0}
                >
                  <Option value={false}>Top 20 Hub</Option>
                  <Option value={true}>Top 20 Agency</Option>
                </Select>
              </Form.Item>
            </Col>
          )}
            ...
            <Form.Item name="search" label=" " >
              <Button
                ...
                onClick={() => {
                  ...
                  console.log(form.getFieldValue('companyId'))
