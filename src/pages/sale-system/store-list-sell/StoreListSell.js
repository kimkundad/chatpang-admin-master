...
const StoreListSell = (props) => {
  ...
  const { orderItemData } = useSelector((state) => state.storeListSellReducer);
  const { isLoading, isMobile } = useSelector((state) => state.mainReducer);

  useEffect(() => {
    window.scrollTo(0, 0);
    dispatch(allAction.storeListSellAction.getOrderItemInAgencyWarehouse())
      .then()
      .catch((e) => message.error(e.message));
    return () => {
      dispatch(allAction.storeListSellAction.clearData())
        .then()
        .catch((e) => message.error(e.message));
    };
  }, []);
  const onSearch = (value) => {
    console.log(value);
    dispatch(allAction.storeListSellAction.getOrderItemInAgencyWarehouse(value))
      .then()
      .catch((e) => message.error(e.message));
  };
          ...
          <Row gutter={[8, 8]}>
            <Col xs={24} sm={18}>
              <Typography.Title level={3}>
                <span className="text-primary">{t('SLS')}</span>
              </Typography.Title>
            </Col>
            <Col sm={24}>
