...
const AddSender = (props) => {
  ...
  const { onCancel, comId, formBig } = props;
                ...
                data.customerAddressData.filter((val) => {
                  if (val.defaultAddress) {
                    ...
                    formBig.setFieldsValue({
                      ...
                      senderNo: val?.no,
                      senderMoo: val?.moo || '',
                      senderAlley: val?.alley || '',
                      senderRoad: val?.road || '',
                      senderDistrictId: val?.districtData?.districtId || '',
                      senderSubdistrictId:
                        val?.subdistrictData?.subdistrictId || '',
                      senderProvinceId: val?.provinceData?.provinceId || '',
                      senderPostcode: val?.postcode || '',
                    });
                  }
                });
      ...
      >
        <Card style={cardStyle} bodyStyle={cardBodyStyle}>
          <Row>
            ...
            <Col {...itemLayout}>
              <Form.Item
                labelCol={{ xs: radioCustomer ? 10 : 8 }}
                label={
                  radioCustomer
