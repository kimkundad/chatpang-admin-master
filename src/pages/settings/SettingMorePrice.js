...
const SettingMorePrice = (props) => {
  const { settingMorePriceData, companyData } = useSelector(
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    dispatch(
      allAction.settingMorePriceAction.getSettingMorePrice(checkNullCompanyId)
    )
      .then(() => {})
  ...
  const handleFilterChange = (value) => {
    ...
    dispatch(allAction.settingMorePriceAction.getSettingMorePrice(value))
      .then(() => {})
      .catch((e) => message.error(e.message));
  };
  ...
  const onFinishSearch = (value) => {
    if (roleLevel === 'SAD') {
      if (value.companyId == null) {
        dispatch(allAction.settingMorePriceAction.filterSettingMorePrice(value))
          .then(() => {})
          .catch((e) => message.error(e.message));
      } else {
        dispatch(allAction.settingMorePriceAction.filterSettingMorePrice(value))
      ...
      dispatch(allAction.settingMorePriceAction.filterSettingMorePrice(obj))
        .then(() => {})
        .catch((e) => message.error(e.message));
    }
  };
          ...
          {isMobile ? (
            FilterPermission('isCreate') && (
              <Button
                type="primary"
                icon={<PlusOutlined />}
                onClick={() => history.push('./setting-more-price/create')}
                block
              >
                {t('create')}
              </Button>
          ...
          <Row gutter={[24, 24]}>
            <Col xs={{ span: 24 }}>
              <Table
                ...
                dataSource={settingMorePriceData}
                pagination={{
                  total: settingMorePriceData?.length || 0,
                ...
                onRow={(record) => ({
                  onClick: () => {
                    history.push(
                      `./setting-more-price/${record.incrementalPriceId}`
                    );
                    dispatch(
              ...
              >
                ...
                <Column
                  title={t('setting-more-price-detail')}
                  width={100}
...
export default SettingMorePrice;
