...
const FilterData = ({ onSearch, pageCode }) => {
  ...
  useEffect(() => {
    ...
    dispatch(allAction.companyAction.getCompanyData())
      .then()
      .catch((e) => message.error(e.message));
    dispatch(allAction.userManagementAction.getHubList())
      .then()
      .catch((e) => message.error(e.message));
    dispatch(allAction.userManagementAction.getAgencyList())
      .then()
      .catch((e) => message.error(e.message));
      ...
      >
        <Row gutter={[8, 4]}>
          {/* {FilterUserLevel(["SAD", "COM", "HUB"]) && (
            <Col flex="1 1 100px">
              <Form.Item
                label={pageCode === "USE" ? t("user-level") : t("role-level")}
                name={pageCode === "USE" ? "userLevel" : "roleLevel"}
              >
                <Select
                  // allowClear
                  defaultValue=""
